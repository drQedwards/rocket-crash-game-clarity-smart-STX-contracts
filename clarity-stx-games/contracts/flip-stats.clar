;; flip-stats - on-chain heads-or-tails counter and top-10 leaderboard.
;;
;; NOT a wagering contract: nothing in this file moves STX or any other asset.
;; It only tracks personal flip counts, the user's last "side" value, and the
;; top-10 by lifetime flip count. The randomness is a deterministic function of
;; block-height and the caller's flip count, which is fine for an informational
;; counter but is NOT safe for real-money wagers. Use the fair-flip-* templates
;; in this project for any wager flow.
;;
;; Lifted from the legacy `Flip.clarinet` file at the repo root and ported to a
;; valid Clarity contract file inside a Clarinet project.
